# Param
export AZURE_DEFAULTS_GROUP=aks-demo-rg
export AZURE_DEFAULTS_LOCATION=SwedenCentral
export CLUSTER=your-aks-demo-cluster

# Create public domain
export DOMAIN_NAME=akms-dev.shop
...
export AZURE_LOADBALANCER_DNS_LABEL_NAME=lb-akms-dev
...
metadata:
    name: helloweb
    annotations:
        service.beta.kubernetes.io/azure-dns-label-name: lb-akms-dev
spec:
    ports:
    - port: 443
      protocol: TCP
...
# TODO
# configure cert-manager to use Let's Encrypt and Azure DNS to create a trusted certificate
